...
async def cmd_start(message: types.Message):
    ...
    await message.answer(f"<b>{check_user}</b>", reply_markup=keyboard, parse_mode=types.ParseMode.HTML)
...
async def cmd_type_spends(message: types.Message):
    ...
    await message.answer(f"Please type a spends\n<i>Examples:\nbar 800\nfood 1000</i>", reply_markup=keyboard, parse_mode=types.ParseMode.HTML)
...
async def cmd_statistics(message: types.Message):
    ...
    spends_clear = spends_.replace('(','').replace(')','').replace("'","")
    await message.answer(f"{spends_clear}\nTotal\n{total}")

@dp.message_handler(Text(equals="Config spends"))
...
@dp.message_handler(Text(equals="Delete category"))
async def cmd_delete(message: types.Message):
    await mesage.answer("here list of categories from db\nChoose one to delete and type")
...
"""
spends_raw = input("Enter spends:")
spends = spends_raw.split()
category = spends[0]
amount = float(spends[1])
print(category, amount)
"""

"""
categories = ["food", "bar", "taxi"]
buttons = ["Type", "Back"]
lst = categories + buttons"""
"""
buttons = ["Type", "Back"]
lst = spends + buttons
for item in lst:
